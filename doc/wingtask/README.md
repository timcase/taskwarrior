...
4. A fork of the develop branch was created using the commit 48fb891 as the base, and this new branch was named wingtask-v3.4.2. ```git checkout -b wingtask-v3.4.2 48fb891```
5. Patches were applied to the branch for the extended behavior of WingTask.

## Future updating

1. Checkout the develop branch of the original Taskwarrior repository and pull the latest changes to ensure that it is up to date with the latest version of Taskwarrior. ```git checkout develop && git pull origin develop```
...
5. ```git checkout -b wingtask-vX.Y.Z <commit-hash>``` to create a new branch for the new version of Taskwarrior, using the commit hash for the new version as the base.
